...
namespace App\Http\Controllers\Iomsapi\V2;

use App\Apikey;
use App\Http\Controllers\Controller;
use App\V2\Inventory;
...
class IomsapiController extends Controller
{
        ...
        if ($this->apiKey) {
            ...
            if (is_array($tableData)) {
                foreach ($tableData as $table) {
                    ...
                    }else if ($table->tableName == 'voertuigen_global_ol') {
                        $this->clientNrs = explode(',', $table->clientNrs);
                    }
                }
            }
    ...
    public function getInventory(Request $request)
    {
        ...
        $items = $inventory->getInventory($this->clientNrs, $limit, $offset, $search, $filters, $filtersBetween, $orderBy, $order);

//        $itemsCollection = $items->get();

        $itemsCount = $inventory->getInventoryCount($this->clientNrs, $search, $filters, $filtersBetween);
    ...
    public function getInventoryItem($hexonId)
    {
        ...
        $item = Inventory::where('hexonnr', $hexonId)->first();
